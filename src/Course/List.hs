...
seqOptional (Full x:.xs) = combine' x $ seqOptional xs
  where
    combine' :: a -> Optional (List a) -> Optional (List a)
    combine' _ Empty = Empty
    combine' y (Full ys) = Full (y :. ys)
--seqOptional =
...
lengthGT4 ::
  List a
  -> Bool
lengthGT4 x = lengthGT4' x 1
  where
    lengthGT4' :: List a -> Int -> Bool
    lengthGT4' Nil _ = False
    lengthGT4' (_:.ys) n
      | n > 4 = True
      | otherwise = lengthGT4' ys (n+1)
--lengthGT4 =
...
reverse ::
  ...
  -> List a
reverse Nil = Nil
reverse x = foldLeft (\ys y -> y:.ys) Nil x
--reverse =
